...
// ============================================
// OPTS.TURKPATENT.GOV.TR İÇİN AKIM
// ============================================

// Sayfa yüklenince otomatik başlat
(function() {
  const url = window.location.href;
  if (/^https:\/\/opts\.turkpatent\.gov\.tr\/trademark/i.test(url)) {
    const hash = window.location.hash;
    const match = hash.match(/#bn=([^&]+)/);
    if (match) {
      const appNo = decodeURIComponent(match[1]);
      console.log('[Evreka OPTS] Başvuru no:', appNo);
      setTimeout(() => runOptsFlow(appNo), 1000);
    }
  }
})();

async function runOptsFlow(appNo) {
  console.log('[Evreka OPTS] Akış başladı:', appNo);
  
  const input = document.querySelector('input[type="text"]');
  if (!input) {
    console.error('[Evreka OPTS] Input bulunamadı');
    return;
  }
  ...
  setTimeout(() => {
    ...
    if (btn) {
      btn.click();
      setTimeout(() => waitForOptsResults(appNo), 1500);
    }
  }, 500);
}

function waitForOptsResults(appNo) {
  let attempts = 0;
  const interval = setInterval(() => {
    attempts++;
    const rows = document.querySelectorAll('tbody tr');
    const validRow = Array.from(rows).find(r => r.querySelectorAll('td').length > 2);
    
    if (validRow) {
      clearInterval(interval);
      scrapeOptsData(appNo, validRow);
    } else if (attempts >= 40) {
      clearInterval(interval);
      console.error('[Evreka OPTS] Timeout');
    }
  }, 500);
}

function scrapeOptsData(appNo, row) {
  const cells = Array.from(row.querySelectorAll('td'));
  const data = {
    applicationNumber: appNo,
    brandName: '',
  ...
  const img = row.querySelector('img');
  if (img?.src) data.imageSrc = img.src;
  
  cells.forEach(cell => {
    const txt = cell.textContent.trim();
    if (/^\d{4}\/\d+$/.test(txt)) data.applicationNumber = txt;
    else if (/^\d{2}\.\d{2}\.\d{4}$/.test(txt) && !data.applicationDate) data.applicationDate = txt;
    else if (/^\d{4}\s+\d+$/.test(txt)) data.registrationNumber = txt;
    else if (/TESCİL|GEÇERSİZ|BAŞVURU/i.test(txt)) data.status = txt;
    else if (txt.includes('/') && /\d+/.test(txt)) data.niceClasses = txt;
    else if (txt && !data.brandName) data.brandName = txt;
    else if (txt && !data.ownerName) data.ownerName = txt;
  });
  
  console.log('[Evreka OPTS] Veri toplandı:', data);
  
  // Background'a gönder
  chrome.runtime.sendMessage({
    type: 'FORWARD_TO_APP',
    messageType: 'VERI_GELDI_OPTS',
    data: [data]
  });
...
// OPTS için mesaj listener
chrome.runtime?.onMessage?.addListener?.((request, sender, sendResponse) => {
  if (request?.type === 'AUTO_FILL_OPTS' && request?.data) {
    console.log('[Evreka OPTS] Mesaj alındı:', request.data);
    if (/opts\.turkpatent\.gov\.tr/i.test(window.location.href)) {
      runOptsFlow(request.data);
    }
    sendResponse?.({ status: 'OK' });
    return true;
  }
