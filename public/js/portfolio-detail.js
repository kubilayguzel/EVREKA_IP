...
// portfolio-detail.js içinde eklenecek kod
...
class PortfolioTurkpatentIntegration {
    ...
    async openWithAutomation(applicationNumber) {
        ...
        try {
            ...
            const waitForLoad = () => {
                ...
                try {
                    // Popup'ın document'ine erişmeye çalış
    ...
    async startAutomation(applicationNumber) {
        ...
        try {
            ...
            // 1. Dosya Takibi sekmesini aktif et
            await this.activateFileTrackingTab(doc);
            
            // 2. Başvuru numarasını doldur
        ...
        return new Promise((resolve, reject) => {
            ...
            const findAndClickTab = () => {
                ...
                if (fileTrackingTab && fileTrackingTab.click) {
                    ...
                    setTimeout(resolve, 1500); // Tab değişimi için bekle
                } else {
    ...
    async fillApplicationNumber(doc, applicationNumber) {
        return new Promise((resolve, reject) => {
            ...
            const findAndFillInput = () => {
                ...
                // Fallback: İlk görünen input'u kullan
                if (!targetInput) {
                ...
                if (targetInput) {
                    ...
                    // React/Modern framework event'lerini tetikle
    ...
    // Yöntem 3: Her iki yöntemi birden kullan
    async queryBothMethods(applicationNumber) {
        const userChoice = confirm(
            'Nasıl sorgulamak istiyorsunuz?\n\n' +
            'TAMAM: Yeni pencerede otomatik sorgu (önerilen)\n' +
            'İPTAL: Robot ile arka plan sorgulama'
        );
    ...
    showNotification(message, type) {
        // Mevcut notification sistemini kullan veya basit alert
    ...
    showResults(html, screenshot, applicationNumber) {
        // Sonuçları modal veya yeni sekme olarak göster
...
// Global instance
const turkpatentIntegration = new PortfolioTurkpatentIntegration();

// Portfolio detay sayfasında kullanım
function addTurkpatentButtons(portfolioData) {
    const container = document.querySelector('.portfolio-actions') || 
                     document.querySelector('.card-footer') ||
                     document.querySelector('.action-buttons');
    
    if (!container || !portfolioData.applicationNumber) return;

    // Ana buton
    const mainButton = document.createElement('button');
    mainButton.className = 'btn btn-outline-primary me-2';
    mainButton.innerHTML = '<i class="fas fa-external-link-alt"></i> TÜRKPATENT Sorgula';
    mainButton.onclick = () => turkpatentIntegration.queryTurkpatent(
        portfolioData.applicationNumber, 
        'both' // Kullanıcı seçimi yapar
    );

    // Dropdown menu (opsiyonel - gelişmiş kullanıcılar için)
    ...
    dropdown.innerHTML = `
        ...
        <ul class="dropdown-menu">
            <li><a class="dropdown-item" href="#" onclick="turkpatentIntegration.queryTurkpatent('${portfolioData.applicationNumber}', 'popup')">
                <i class="fas fa-window-restore"></i> Yeni Pencere (Otomatik)
            </a></li>
            <li><a class="dropdown-item" href="#" onclick="turkpatentIntegration.queryTurkpatent('${portfolioData.applicationNumber}', 'api')">
                <i class="fas fa-robot"></i> Robot Sorgu (Arka Plan)
            </a></li>
            <li><hr class="dropdown-divider"></li>
            <li><a class="dropdown-item" href="https://www.turkpatent.gov.tr/arastirma-yap?form=trademark" target="_blank">
                <i class="fas fa-link"></i> Manuel Sorgu (Direkt Git)
            </a></li>
        </ul>
    `;
...
// Portfolio detay sayfası yüklendiğinde otomatik çalıştır
document.addEventListener('DOMContentLoaded', function() {
    // Portfolio verisi sayfada mevcutsa butonları ekle
    if (typeof portfolioData !== 'undefined' && portfolioData.applicationNumber) {
        setTimeout(() => {
            addTurkpatentButtons(portfolioData);
        }, 1000);
    }
});
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { PortfolioTurkpatentIntegration, turkpatentIntegration };
}
