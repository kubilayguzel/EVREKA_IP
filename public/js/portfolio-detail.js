...
// YALNIZCA tp-sorgu-eklentisi'ni hedefleyen fonksiyon
window.triggerTpQuery = function(applicationNo){
  ...
  console.log('[TP Query] Başvuru numarası:', appNo);

  // SADECE tp-sorgu-eklentisi ID'si
  const EXT_ID_TP = 'bbcpnpgglakoagjakgigmgjpdpiigpah';

  // Eklenti kontrolü
  if (typeof chrome === 'undefined' || !chrome.runtime) {
    alert('Chrome eklentisi ortamı bulunamadı. Lütfen Chrome tarayıcısı kullanın.');
    return;
  }
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      if (hasErr) {
        ...
        console.error('[TP Query] Eklenti hatası:', errMsg);
        alert(`Eklenti çalışmıyor. Lütfen "TP - Marka Dosya Sorgu" eklentisinin yüklü ve aktif olduğundan emin olun.\n\nHata: ${errMsg}`);
        return;
      }
      
      if (response && response.status === 'OK') {
        console.log('[TP Query] Eklenti başarıyla yanıt verdi');
      } else {
        console.warn('[TP Query] Beklenmeyen eklenti yanıtı:', response);
      }
    }
  );
};
